...
class readmeteo(object):
    ...
    **Functions**
    ...
    def initial(self):
        ...
        name = cbinding('RunoffMaps') #Peter Greve Test
        nameall = glob.glob(os.path.normpath(name)) #Peter Greve Test
        if not nameall:
            msg = "Error 215: In readmeteo, cannot find runoff maps " #Peter Greve Test
            raise CWATMFileError(name, msg, sname='RunoffMaps') #Peter Greve Test
        namemeteo = nameall[0]
        ...
        self.var.QMaps = 'RunoffMaps' #Peter Greve Test
        self.var.totETMaps = 'totETMaps' #Peter Greve Test
        self.var.GWMaps = 'GWMaps' #Peter Greve Test
        self.var.openwaterEMaps = 'openwaterEMaps' #Peter Greve Test
        ...
        if checkOption('no_meteo'): #Peter Greve Test
            meteomaps = [self.var.QMaps, self.var.GWMaps, self.var.SMMaps, self.var.eva0Maps] #Peter Greve Test
        else:		
...
# --------------------------------------------------------------------------
    ...
    def dynamic(self):
        ...
        if checkOption('no_meteo'): # Peter Greve test
            ...
            #self.var.runoff = self.var.runoff * self.var.DtDay * self.var.con_precipitation
            self.var.runoff = np.maximum(0., self.var.runoff)
            # read totalET
            #self.var.totalET, MaskMapBoundary = readmeteodata(self.var.totETMaps, dateVar['currDate'], addZeros=True, mapsscale = self.var.meteomapsscale, buffering= self.var.buffer)
            #self.var.totalET = np.maximum(0., self.var.totalET)
            # read gwRecharge
            ...
            # read open water evap
            #self.var.sum_openWaterEvap, MaskMapBoundary = readmeteodata(self.var.openwaterEMaps, dateVar['currDate'], addZeros=True, mapsscale=self.var.meteomapsscale)
            #self.var.sum_openWaterEvap = np.maximum(0., self.var.sum_openWaterEvap)
            # open Water potential evap
